...
- A document named `*.VIDEO` is a recording's transcript: `SCREEN:` lines are
  OCR (0/O and 1/I confused); `SAID (captions):` lines come from the meeting's
  own caption file; `SAID (recogniser - unreliable):` lines come from the
  Windows speech recogniser and may be sentences nobody said - never quote
  those. Quote the others as "said in the recording at 00:12:05", never as
  fact.
