...
from typing import AbstractSet, Callable, Dict, FrozenSet, List, Optional, Sequence, Set, Tuple, TypeVar
...
class Ctx:
    def __init__(self, conn: sqlite3.Connection, quiet: bool = False):
        ...
        # the copybooks IBM supplies (expand.IBM_COPYBOOKS) and the names the manifest's `system_includes` adds, name ->
        # product: a COPY of one no member carries is IBM-supplied, not in the estate (supplied_names, ROADMAP re-parse
        # item 27); `manifest_includes` the manifest's own names, recorded with the run
        self.system_includes: Dict[str, str] = dict(expand.IBM_COPYBOOKS)
        self.manifest_includes: List[str] = []
...
def open_db(path: str, rebuild: bool = False) -> sqlite3.Connection:
    ...
    for table, col, decl in (("doc_section", "ordinal", "INTEGER"), ("doc_image", "extracted_path", "TEXT"),
                             ...
                             ("build_run", "system_includes", "TEXT"),
...
_MEMBER_NAME = re.compile(r"[A-Z0-9@#$][A-Z0-9@#$\-_]{0,9}")


def load_system_includes(ctx: Ctx, manifest_path: Optional[str]) -> None:
    """`system_includes` from the manifest: copybook names the shop's
    compiles read from a product library it does not keep among its own
    (another vendor's, or an IBM product's this toolkit does not list), added
    to expand.IBM_COPYBOOKS in ctx.system_includes. A COPY of one of them
    that no member of the index carries is IBM-supplied, not in the estate:
    recorded with no member, never a gap (ROADMAP re-parse item 27). A name
    that is not a member name is left out. Read before the parse, beside
    load_declared_kinds; a manifest change re-parses every member, so the
    programs follow a name added or taken away. sources.json's
    `system_includes` reaches a manifest the UI writes (fetch.write_manifest)."""
    ctx.system_includes = dict(expand.IBM_COPYBOOKS)
    ctx.manifest_includes = []
    if not manifest_path or not os.path.isfile(manifest_path):
        return
    try:
        with open(manifest_path, "r", encoding="utf-8") as fh:
        ...
        return
    names = man.get("system_includes") if isinstance(man, dict) else None
    if not isinstance(names, list):
        return
    for n in names:
        name = str(n).strip().upper()
        if _MEMBER_NAME.fullmatch(name) and name not in ctx.manifest_includes:
            ctx.manifest_includes.append(name)
            ctx.system_includes.setdefault(name, expand.MANIFEST_PRODUCT)
    ctx.manifest_includes.sort()


def supplied_names(ctx: Ctx) -> FrozenSet[str]:
    """The names of ctx.system_includes that no member of the index carries,
    of any kind: a COPY of one is IBM-supplied, not in the estate. A member
    of the name - a copy the shop keeps, a stub, one filed as another kind -
    makes it an ordinary copybook: expanded when the resolver takes it, NOT
    FOUND (with the misfiled advice) when it does not. By name after the
    inventory, so every program of a build reads the same set."""
    return frozenset(n for n in ctx.system_includes if not ctx.by_name.get(n))
...
def index_cobol(ctx: Ctx, mem: Mem) -> None:
    ...
    # a COPY of a copybook IBM supplies that no member carries (DFHAID, CMQV ...) is recorded with no member and no
    # warning: the compile reads it from the product's library, so it makes nothing partial (ROADMAP re-parse item 27)
    exp = expand.expand(lines, mem.id, make_resolver(ctx, mem, notes), supplied=supplied_names(ctx))
...
# said of a COPY inside a copybook whose text is not in the copybook's own layout (a 'layout_warning' row)
NESTED_NOT_COUNTED = "its bytes are not in this layout: an item after it in the same record sits further on by its length"
_WARN_LINE = re.compile(r"^L(\d+):")


def copybook_own_fields(exp: expand.Expansion, roots: List[copybook.Field]) -> List[copybook.Field]:
    """The copybook's OWN items of a layout computed over its expanded text:
    every item written in its own lines (depth 0), at the offset the nested
    copybooks' bytes put it, and at its own line; an OS/VS `01 X COPY Y.`
    item at the line that names it. A nested copybook's items are its own
    member's rows, cited there - never stored under this one. An own item
    under a nested group hangs from its nearest own ancestor."""
    renamed = set(exp.renamed)
    copy_line = {rid: ln for (_n, _l, _r, ln, rid) in exp.own_copies if rid is not None}
    kept: List[copybook.Field] = []
    own_ids: Set[int] = set()
    for f in copybook.flatten(roots):
        if f.name == copybook.SYNTHETIC_ROOT:
            continue
        mid, src_line, depth = exp.origin(f.line)
        if depth == 0 and src_line is not None:
            f.line = src_line
        elif f.line in renamed and mid in copy_line:
            f.line = copy_line[mid]                    # `01 X COPY Y.`: X is written on the COPY line
        else:
            continue
        conds = []
        for (cname, vals, ln) in f.conds:                # an 88 at its own line; one a nested COPY brings is that
            _m, cl, cd = exp.origin(ln)                  # copybook's text (and none of its rows either, as before)
            if cd == 0 and cl is not None:
                conds.append((cname, vals, cl))
        f.conds = conds
        kept.append(f)
        own_ids.add(id(f))
    for f in kept:
        p = f.parent
        while p is not None and id(p) not in own_ids and p.name != copybook.SYNTHETIC_ROOT:
            p = p.parent
        f.parent = p if p is not None and id(p) in own_ids else None
    return kept


def nested_copy_notes(exp: expand.Expansion, system_includes: Dict[str, str]) -> List[Tuple[str, Optional[int]]]:
    """(detail, own line) of a 'layout_warning' for every COPY inside a
    copybook - at any depth - whose text is not in its layout: NOT FOUND,
    skipped (recursive, nested too deep), a stub, IBM-supplied and not in
    the estate. The expander's words are a program's; the copybook's say
    what it costs its layout (NESTED_NOT_COUNTED). The line is the
    copybook's own for a COPY in its own lines, None for one deeper."""
    out: List[Tuple[str, Optional[int]]] = []
    for w in exp.warnings:
        head = (w.replace(" - fields/code from it are missing from this program's facts", "")
                .replace("; " + expand.STUB_COMPILED, ""))
        m = _WARN_LINE.match(head)
        out.append((f"{head} - {NESTED_NOT_COUNTED}", int(m.group(1)) if m else None))
...
def _warning_at_own_line(exp: expand.Expansion, w: str) -> str:
    """A layout warning of the expanded text ('L12: SYNC on X ...') with the
    line of the member it was written in: the copybook's own, or 'L3 of COPY
    Y' for an item a nested copybook brought in."""
    m = _WARN_LINE.match(w)
    if not m:
        return w
    run = exp.run_at(int(m.group(1)))
    _mid, src, depth = exp.origin(int(m.group(1)))
    if src is None:
        return w
    where = f"L{src}" if not depth else f"L{src} of COPY {run.via_copy if run and run.via_copy else '?'}"
    return where + w[m.end() - 1:]


def index_copybook(ctx: Ctx, mem: Mem) -> None:
    conn = ctx.conn
    lines = ctx.lines_for(mem)
    # The layout is computed over the copybook's text with every COPY in it put in place by the resolver the programs
    # use (ROADMAP re-parse item 27): the nested copybook's bytes sit inside this record, so every item after the COPY
    # is at its real offset. Before, the copybook's own lines alone were read and each item after a nested COPY sat
    # its length too early in `layout` and `field` - the program's own view was right (tools/synth/repro/F06; the
    # synthetic estate's F28, F29, F32, F33: 58 offsets 121 bytes short). A copybook copying itself is 'skipped -
    # recursive'. Which of several same-named copies was expanded is the resolver's choice, as for a program; it is
    # said by `layout` from the copy_use row (the programs' 'ambiguous_copybook' rows are checked against their
    # listings, which a copybook has none of).
    choices: List[Tuple[str, str, int]] = []
    exp = expand.expand(lines, mem.id, make_resolver(ctx, mem, choices), stack=(mem.name.upper(),),
                        supplied=supplied_names(ctx))
    logical = reader.join_cobol_continuations(exp.lines)
    roots, warns = copybook.parse_data_division(logical)
    fields = copybook_own_fields(exp, roots)
    _insert_fields(conn, mem.id, fields)

    # a group's assembled VALUE only of the copybook's own pieces: a group a nested COPY fills is that copybook's text
    own = {id(f) for f in fields}
    lits: List[Tuple[str, str, str, int]] = list(_group_value_literals(
        [f for f in fields if all(id(c) in own for c in f.children)]))
    for f in fields:
        if f.value_lit:
    ...
    # each COPY row names the member the layout above expanded for it (ROADMAP re-parse item 27): the parser's COPYs
    # of a name and the expander's are the same statements in the same order, so the k-th of each is paired (an `01 X`
    # / `COPY Y.` statement starts on the line before the COPY, so the lines may differ); by name and line when the
    # counts differ. Before the item a copybook's rows were never resolved (LESSONS 184); only a PROGRAM's row says
    # whether a COPY was found, and the scans that read it take programs only, so nothing reads these as one
    by_line = {(n, ln): rid for (n, _l, _r, ln, rid) in exp.own_copies}
    expanded: Dict[str, List[Optional[int]]] = {}
    for (n, _l, _r, _ln, rid) in exp.own_copies:
        expanded.setdefault(n, []).append(rid)
    parsed: Dict[str, int] = {}
    for (n, _l, _r, _ln) in facts.copies:
        parsed[n.upper()] = parsed.get(n.upper(), 0) + 1
    rows = []
    seen: Dict[str, int] = {}
    for (n, lib, rep, ln) in facts.copies:
        up = n.upper()
        k = seen.get(up, 0)
        seen[up] = k + 1
        rid = (expanded[up][k] if len(expanded.get(up, ())) == parsed[up] else by_line.get((up, ln)))
        rows.append((mem.id, n, lib, rep, ln, rid))
    conn.executemany(
        "INSERT INTO copy_use(member_id,copybook,of_library,replacing,line,resolved_member_id) VALUES(?,?,?,?,?,?)", rows)
    ...
    for w in warns:
        if "SYNC" in w or "not compile" in w or "mixed" in w:
            conn.execute("INSERT INTO unresolved(member_id,kind,detail,line) VALUES(?,?,?,?)",
                         (mem.id, "layout_warning", _warning_at_own_line(exp, w), None))
    # a COPY in a data copybook whose text is not in the layout (NOT FOUND, skipped, a stub, IBM-supplied): the offsets
    # after it are short by its length - said on the copybook, which stays `ok` (its own items are all there; each
    # program copying it carries its own NOT FOUND and is partial for it). A procedure copybook has no layout to say it of
    if fields:
        conn.executemany("INSERT INTO unresolved(member_id,kind,detail,line) VALUES(?,?,?,?)",
                         [(mem.id, "layout_warning", d, ln) for d, ln in nested_copy_notes(exp, ctx.system_includes)])
...
def _build(args: argparse.Namespace, progress: Progress, t0: float) -> int:
    ...
    last = conn.execute("SELECT fingerprint, manifest_sha, declared_kinds, system_includes FROM build_run "
    ...
    # the copybook names the manifest adds to the IBM-supplied ones (ROADMAP re-parse item 27): recorded with the run,
    # so `program`, `copybook`, `coverage` and atlas.recover say such a COPY is not in the estate without the manifest
    load_system_includes(ctx, args.manifest)
    includes_json = json.dumps(ctx.manifest_includes)
    force_all = False
    ...
    run = conn.execute("INSERT INTO build_run(started_at,root,tool_version,fingerprint,manifest_sha,declared_kinds,"
                       ...
                       (time.strftime("%Y-%m-%dT%H:%M:%S"), args.root, VERSION,
                        ...
                        last["system_includes"] if force_all else includes_json))
    ...
    try:
        ...
        return 130
    conn.execute("UPDATE build_run SET fingerprint=?, manifest_sha=?, declared_kinds=?, system_includes=? WHERE id=?",
                 (fp, man_sha, kinds_json, includes_json, run_id))
    conn.commit()
