...
> **The verifier's second round on the query side (LESSONS 245-247, row 242 extended).** F50's double row came back for an instream PROC: its own FTP step has its row on the job's member, beside the job's run of it, and the rule that leaves out a PROC's own step while a job runs the PROC tested the member's kind, not the step. It is decided per step now, and the own step of an instream PROC no job runs is named by the PROC (LESSONS 245). `field` printed the hidden-rows sentence with no sort-card table above it, saying the jobs 'are listed' when none was, and `dataset` did the same for a PROC's default TEST.* name: such a page now says that no indexed job's step or card is there and names the PROC step it left out (LESSONS 246). The Files table's '(no DSN: DUMMY, SYSOUT or instream)' was also printed for a referback the index could not follow; that DD reads `*.S1.X010.OUT (referback not followed)` (LESSONS 242). A PROC's own FTP step named its host as the symbolic `&HOST`; the peer is now said with the PROC's default, `&HOST (PROC default qzlone.example)`, on the query side (LESSONS 247, the choice in ROADMAP). verify.py: F12 F13 F17 FIXED (5 checks).
